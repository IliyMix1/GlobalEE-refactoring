...
@users_router.get('/')
async def get_users(session: AsyncSession = Depends(get_session), user = Depends(get_current_user), role = Depends(check_role)):
    #Вызываем асинхронную функцию, чтобы посмотреть всю таблицу
    return await select_all_records(model=User, session=session)

@users_router.patch('/{user_id}')
async def patch_user(user_id: int, data: UserPatch, session: AsyncSession = Depends(get_session), user = Depends(get_current_user), role = Depends(check_role)):
